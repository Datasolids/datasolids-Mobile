// Vitals list — typed clinical endpoint, scoped to Observation
// category=vital-signs. Each row is one reading (heart rate, BP, temp,
// etc.) with value + unit + reference range + interpretation pill.
...
const Color _kAccent = Color(0xFF16A34A); // green — matches the My Pod tile


class VitalsListScreen extends ConsumerStatefulWidget {
  const VitalsListScreen({super.key});

  @override
  ConsumerState<VitalsListScreen> createState() => _VitalsListScreenState();
}

class _VitalsListScreenState extends ConsumerState<VitalsListScreen> {
  ...
  void _maybeLoadMore() {
    if (_scroll.position.pixels >=
      ...
      ref.read(vitalsControllerProvider.notifier).loadMore();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final notifier = ref.read(vitalsControllerProvider.notifier);
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: _FilterChips(
                    active: state.interpretationFilter,
                    onTap: notifier.setInterpretationFilter,
                  ),
                ),
              ),
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (context, i) => _VitalRow(
                        item: state.items[i],
                        onTap: () => context
                            .push('/pod/clinical/observation/${state.items[i].id}'),
                      ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                total == 1 ? '1 reading' : '$total readings',
                style: TextStyle(
                  fontSize: 13,
...
// ─────────────────────────────────────────────────────────────────
// Filter chips: All / Normal / Abnormal
// ─────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        _Chip(label: 'Normal',   selected: active == 'normal',   onTap: () => onTap('normal')),
        const SizedBox(width: 10),
        _Chip(label: 'Abnormal', selected: active == 'abnormal', onTap: () => onTap('abnormal')),
      ],
    );
  }
}
...
class _VitalRow extends StatelessWidget {
  const _VitalRow({required this.item, required this.onTap});
  final ObservationRow item;
  final VoidCallback onTap;
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      item.name.isEmpty ? 'Observation' : item.name,
                      maxLines: 1,
              ...
              if (item.interpretation.isNotEmpty)
                _SeverityPill(severity: item.severity, label: item.interpretation),
  ...
  static String _valueLine(ObservationRow r) {
    if (r.value == null || r.value!.isEmpty) return r.status;
    return [r.value, r.unit].where((s) => (s ?? '').isNotEmpty).join(' ');
  }
}
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Connect a provider that has vital signs to populate this list.',
